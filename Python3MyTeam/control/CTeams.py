...
class CTeams():
    ...
    def new_team(self):
        ...
        sid = self.sstudent.get_sid_by_uid(uid)
        # 写入团队创始人信息
        add_team_student = self.steams.add_student_in_team(str(uuid.uuid4()), teid, sid, 1000, 1101)
        ...
        if self.judgeData.inData("Students", data):
            for row in data["Students"]:
                if not self.judgeData.inData("Sno", row) or not self.judgeData.inData("Suniversity", row):
                    ...
                    add_team_student_list = self.steams.add_student_in_team(str(uuid.uuid4()), teid, sid, 1002, 1100)
                    if not add_team_student_list and add_status:
                        add_status = False
                    uid2 = self.sstudent.get_uid_by_sid(sid)  # 根据students中所有的sid来获取
                    add_infor = self.sinfor.add_infor(str(uuid.uuid4()), uid, NEW_INVITATION, 1200, 901, cid, teid, uid2) # 这里需要判断一下分步异常的问题
        ...
        if self.judgeData.inData("Teachers", data):
            for row in data["Teachers"]:
                if not self.judgeData.inData("Tno", row) or not self.judgeData.inData("Tuniversity", row):
                    ...
                    add_team_teacher_list = self.steams.add_teacher_in_team(str(uuid.uuid4()), teid, tid, 1100)
    ...
    def add_student(self):
        ...
        # 创建学生团队关联，创建通知信息
        response_of_student_and_team = self.steams.add_student_in_team(str(uuid.uuid4()), teid, sid, 1002, 1100)
        if not response_of_student_and_team and add_status:
            add_status = False

        response_of_infor = self.sinfor.add_infor(str(uuid.uuid4()), uid, NEW_INVITATION, 1200, 901, cid, teid, uid2)
    ...
    def invate_add_team(self):
        ...
        response_of_infor = self.sinfor.add_infor(str(uuid.uuid4()), uid, NEW_REQUEST, 1200, 905, cid, teid, uid2)
        if not response_of_infor and add_status:
            add_status = False
    ...
    def add_teacher(self):
        ...
        # 创建教师团队关联，创建通知信息
        response_of_student_and_team = self.steams.add_teacher_in_team(str(uuid.uuid4()), teid, tid, 1100)
        if not response_of_student_and_team and add_status:
            add_status = False
        response_of_infor = self.sinfor.add_infor(str(uuid.uuid4()), uid, NEW_INVITATION, 1200, 901, cid, teid, uid2)
        if not response_of_infor and add_status:
    ...
    def sub_student(self):
        ...
        if sub_index == 1102:
            ...
            response_of_infor = self.sinfor.add_infor(str(uuid.uuid4()), uid, message, 1200, 903, None, teid, uid2)
            if not response_of_update_tstudent and add_status:
                add_status = False
