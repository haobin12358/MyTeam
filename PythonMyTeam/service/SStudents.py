...
class SStudents():
    ...
    def get_student_tech_by_sid(self, sid):
        student_tech = self.session.query(model.STechs.Sid, model.STechs.STname, model.STechs.STlevel)\
            .filter_by(Sid = sid).all()
        return student_tech

    #获取学生竞赛简历
    def get_student_use_by_sid(self, sid):
        student_use = self.session.query(model.SCuse.Sid, model.SCuse.SCname, model.SCuse.SCno)\
            .filter_by(Sid = sid).all()
        return student_use